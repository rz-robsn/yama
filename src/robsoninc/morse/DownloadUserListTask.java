	...
	protected JSONArray doInBackground(Object... params) {
		...
		try {
			...
			nameValuePairs.add(new BasicNameValuePair("registration_id",
					prefs.getString(Constants.C2DM_ID_KEY, "")));
			nameValuePairs.add(new BasicNameValuePair("user_id", prefs.getString(Constants.USER_ID_KEY, "")));
			httppost.setEntity(new UrlEncodedFormEntity(nameValuePairs));

			// Execute HTTP Post Request and retrieve response
			HttpResponse r = httpclient.execute(httppost);
			HttpEntity resEntity = r.getEntity();			
